...
omnibus_tests <- function(df, outcomes, predictors, var_labels) {
  ...
  for (predictor in predictors) {
    ...
    for (outcome in outcomes) {
      ...
      results_list[[paste0(outcome, "__", predictor)]] <- results_table
      
    
      # Generate the dynamic title using variable labels
      title_text <- paste0(
  ...
  for (key in names(results_list)) {
    ...
    table <- table %>%
      mutate(
        `Adjusted p-value` = round(adjusted_p_values[adj_index:(adj_index + nrow(table) - 1)], 3)
      )
    # mutate(`Adjusted p-value` = round(adjusted_p_values[adj_index:(adj_index + nrow(table) - 1)]), 3)
