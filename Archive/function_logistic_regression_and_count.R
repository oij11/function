...
analyze_multiple_variables <- function(var_list, data, group_var) {
  ...
  for (var_name in names(var_list)) {
    ...
    freq_table <- data_filtered %>%
      ...
      select(!!sym(group_var), !!sym(var_name), formatted) %>%
      # wide format: one row per level of the var_name, columns = groups
      pivot_wider(
    ...
    #    We want the row for the second level of the grouping variable
    ...
    model_tidy <- broom::tidy(model, conf.int = TRUE) %>%
      filter(grepl(group_var, term)) %>%      # ensure we pick the group_var row
      mutate(
        OR        = round(exp(estimate), 2),
        Conf.Low  = round(exp(conf.low), 2),
        Conf.High = round(exp(conf.high), 2),
      ...
      # We'll join on the second factor level of the var_name so it appears on that row only
      transmute(
        !!sym(var_name) := levels(data_filtered[[var_name]])[2],
    ...
    # Store in a list to add p-values later
    results_list[[var_name]] <- result_table
  }
  
  # 5) Adjust p-values across all models
  #    Collect, adjust, and assign
  all_p_values <- unlist(lapply(results_list, function(tbl) tbl$`p-value`[!is.na(tbl$`p-value`)]))
  if (length(all_p_values) > 0) {
    ...
    for (nm in names(results_list)) {
      tbl <- results_list[[nm]]
      # If there's exactly one row that has a non-NA p-value, we assign it
      # Because each logistic regression only yields 1 row of effect
      ...
      if (length(has_p) == 1) {
        ...
        # no p-values -> fill column with NA or skip
        tbl$`Adjusted p-value` <- NA
      }
      results_list[[nm]] <- tbl
    }
  } else {
    # if no p-values at all, just fill with NA
    for (nm in names(results_list)) {
