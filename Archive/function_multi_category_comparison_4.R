...
omnibus_tests <- function(df, outcomes, predictors, var_labels) {
  ...
  for (predictor in predictors) {
    ...
    for (outcome in outcomes) {
      ...
      chisq_test <- tryCatch(chisq.test(tab), error = function(e) list(statistic = NA, p.value = NA))
      fisher_test <- tryCatch(fisher.test(tab), error = function(e) list(p.value = NA))
      ...
      results_table <- tibble(
        ...
        `p-value`        = c(chisq_test$p.value, fisher_test$p.value, try_model$lrt_p_value)
      )
      
      # Use a key based on outcome and predictor names
      key <- paste0(outcome, "__", predictor)
      results_list[[key]] <- results_table
      
      # Optional: print a dynamic title and table to the console
      title_text <- paste0("Omnibus Test for differences between ", 
                           var_labels[[predictor]], " for ", var_labels[[outcome]])
      cat("\n\n", title_text, "\n", strrep("-", nchar(title_text)), "\n")
      print(knitr::kable(results_table, format = "pipe"))
    }
  }
...
# Function to adjust p-values for all tests across the results
adjust_all_p_values <- function(results_list, method = "fdr") {
  # Collect all raw p-values from each table
  all_p_values <- unlist(lapply(results_list, function(tbl) tbl$`p-value`))
  
  # Adjust all p-values using the specified method (default is FDR)
  adjusted_values <- p.adjust(all_p_values, method = method)
  
  # Insert the adjusted p-values back into each table
  idx <- 1
  results_list_adjusted <- lapply(results_list, function(tbl) {
    n <- nrow(tbl)
    tbl$`Adjusted p-value` <- round(adjusted_values[idx:(idx + n - 1)], 3)
    idx <<- idx + n
    return(tbl)
  })
  
  return(results_list_adjusted)
}
